...
- **`shared/`** — Domain logic, no DOM access. Designed to eventually work in both browser and server.
  ...
  - `gene-registry.js` — `GENE_REGISTRY` array (19 gene definitions with tier, type, range, mutation params), `TIER_THRESHOLDS` (Primordial/Awakening/Flourishing at 0/50/150 DNA), and tier helper functions. Single source of truth for gene metadata.
