...
- **`shared/`** — Domain logic, no DOM access. Designed to eventually work in both browser and server.
  - `tiles.js` — `TILES` enum (FLOOR=0 through DOOR=16). Single source of truth for tile types. Grid tiles 0-5, overlay tiles 6-16 (collectible, ice, exit, spikes, teleporter, gates, key, door).
  ...
  - `gene-registry.js` — `GENE_REGISTRY` array (26 gene definitions with tier, type, range, mutation params), `TIER_THRESHOLDS` (Primordial/Awakening/Flourishing at 0/50/150 DNA), and tier helper functions. Single source of truth for gene metadata.
  - `solver.js` — Forward BFS solver for Sokoban levels. Verifies solvability by searching normalized (player, boxes) state space. Handles standard pushes, box-ice slides, exit reachability, teleporter traversal, and one-way gate constraints. Used to validate box-ice levels where reverse-play solvability guarantee breaks down.
  - `decorator.js` — Post-generation level decorator. Places overlays (collectibles, teleporters, key/door pairs, ice, exit, spikes, gates) and entities (patrol enemies) on floor tiles. Two tile pools: all-floor (collectibles, spikes, exit) and safe-only (ice, gates).
  ...
  - `genome.js` — `Genome`, `Population`, and `Bot` classes. Genome carries 26 genes across 3 tiers: Tier 1 (structural + style + visual + collectibles = 12), Tier 2 (ice + box-ice + exit + teleporters = 6), Tier 3 (spikes + patrol + gates + keys/doors = 8). `randomGenes(tier)`, `crossover(p1, p2, tier)`, `mutate(rate, tier)` all respect tier gating. Box-ice levels use solver verification to ensure solvability. Population implements tournament selection, crossover, mutation, elitism, and wild card injection. Bot wraps a Genome with procedurally generated name/personality/colors/sprite and a curation system (affinity-based genome preference, 26-dimension distance).
